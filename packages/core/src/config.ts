...
const envSchema = z.object({
  ...
  SCRAPE_RATE_LIMIT_COOLDOWN_MS: z.coerce.number().int().min(0).default(90000),
  SCHEDULER_TICK_SECONDS: z.coerce.number().int().positive().default(30),
  MARKET_SCAN_ENABLED: z.coerce.boolean().default(true),
  MARKET_SCAN_TICK_SECONDS: z.coerce.number().int().positive().default(60),
  MARKET_SCAN_PAGE_SIZE: z.coerce.number().int().min(1).max(120).default(120),
  MARKET_SCAN_MAX_PAGES_PER_JOB: z.coerce.number().int().positive().default(200),
  MARKET_SCAN_FAILURE_BACKOFF_SEC: z.coerce.number().int().positive().default(900),
  EBAY_SCAN_ENABLED: z.coerce.boolean().default(false),
  ...
  EBAY_SCAN_TICK_SECONDS: z.coerce.number().int().positive().default(60),
  EBAY_SCAN_PAGE_SIZE: z.coerce.number().int().min(1).max(200).default(200),
  EBAY_SCAN_MAX_PAGES_PER_JOB: z.coerce.number().int().positive().default(50),
  EBAY_SCAN_FAILURE_BACKOFF_SEC: z.coerce.number().int().positive().default(900),
  EBAY_SEARCH_MIN_DELAY_MS: z.coerce.number().int().min(0).default(1000),
  EBAY_RATE_LIMIT_COOLDOWN_MS: z.coerce.number().int().min(0).default(90000),
  ARBITRAGE_ALERTS_ENABLED: z.coerce.boolean().default(false),
  ARBITRAGE_ALERT_CHECK_SECONDS: z.coerce.number().int().positive().default(120),
  ARBITRAGE_ALERT_TOPIC_NAME: z.string().default(""),
  ARBITRAGE_CACHE_TTL_SEC: z.coerce.number().int().min(0).default(300),
  ARBITRAGE_FX_JPY_PER_USD: z.coerce.number().min(0).default(0),
